...
webview-ui/src/               — React + TypeScript (Vite)
  ...
  hooks/
    ...
    useDayNight.ts            — Day/night state, mode/hemisphere persistence, 30s update interval
  components/
    BottomToolbar.tsx          — + Agent, Layout, Community, Pets, Settings buttons
    ZoomControls.tsx           — +/- zoom (top-right)
    SettingsModal.tsx          — Centered modal: preferences (sound, debug) + appearance (time mode, hemisphere)
...
**Layout model**: `{ version: 1, cols, rows, tiles: TileType[], furniture: PlacedFurniture[], tileColors?: FloorColor[] }`. Grid dimensions are dynamic (not fixed constants). Persisted via debounced saveLayout message → `writeLayoutToFile()` → `~/.pixel-office/layout.json`.
...
**Time engine** (`dayNightCycle.ts`): Maps wall clock hour to 5 periods — night, sunrise, day, sunset, evening. Boundaries defined by `DN_*` constants in `constants.ts`. Seasonal offset via sinusoidal function based on month + hemisphere (±1.5h shift on sunset/sunrise times). Southern hemisphere support auto-detected from browser timezone via `Intl.DateTimeFormat().resolvedOptions().timeZone`.

**Time modes**: `REAL` (wall clock with seasons), `FIXED_DAY`, `FIXED_NIGHT`, `FIXED_SUNSET`, `FIXED_SUNRISE`. Configured in Settings modal, persisted in localStorage.

**Renderer** (`dayNightRenderer.ts`): Two-pass canvas overlay. Pass 1: full-screen color tint per period (`source-over` blend, e.g. night = `rgba(20,15,45,0.55)`). Pass 2: radial light glows on furniture tagged as light emitters (`screen` blend mode). `LIGHT_EMITTERS` map: lamp/wall_lamp → warm amber (`rgba(255,200,100)`), pc/monitor/crt_monitor/laptop → cool blue (`rgba(100,160,255)`), coffee_machine → warm. Glow radius = `DN_GLOW_RADIUS_TILES` (3 tiles), alpha scales with period darkness (`DN_GLOW_MAX_ALPHA` = 0.45 at night, 0 during day).
...
**Gallery flow**: The server proxies `gallery.json` from the `pixel-office-community` GitHub repo. Layout preview screenshots are fetched via GitHub API. The frontend (`GalleryModal.tsx`) renders browsable cards with one-click import.

**Share flow**: ShareModal generates a pre-filled GitHub Issue in `pixel-office-community`. A CI bot processes the submission, creates a PR with an auto-generated preview screenshot (using `?screenshot` mode), and merges on approval.
...
**Vote persistence**: The `votes` field in `gallery.json` is updated every 6 hours by the `update-votes.yml` GitHub Actions workflow in `pixel-office-community`, ensuring counts stay fresh without requiring real-time queries.
