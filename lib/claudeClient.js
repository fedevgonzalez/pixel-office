...
/**
 * Specialized helper: ask claude-service for a pet utterance (5-12 words,
 * Spanish rioplatense, in character). Hits the /pixel-office/pet-speak
 * endpoint which has its own system prompt and personality table.
 *
 * @param {object} opts
 * @param {string} opts.petName
 * @param {'cat'|'dog'} opts.species
 * @param {'lazy'|'playful'|'chill'|'energetic'} [opts.personality]
 * @param {string} [opts.recentActivity]  short summary (≤800 chars)
 * @param {string} [opts.mood]
 * @returns {Promise<string|null>}
 */
async function petSpeak(opts) {
  const body = {
  ...
  const res = await postJson(SERVICE_URL + '/pixel-office/pet-speak', body, 30_000);
  if (!res || !res.success) return null;
  return String(res.result || '').trim() || null;
}

module.exports = { askClaude, classifyTool, petSpeak, MODELS, SERVICE_URL };
