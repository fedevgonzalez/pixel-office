...
import { FILE_WATCHER_POLL_INTERVAL_MS, PROJECT_SCAN_INTERVAL_MS, AUTO_DETECT_MAX_AGE_MS } from './constants.js';
...
export function ensureProjectScan(
	...
	const dirsToScan = allProjectDirs && allProjectDirs.length > 0 ? allProjectDirs : [projectDir];
	...
	projectScanTimerRef.current = setInterval(() => {
		for (const dir of dirsToScan) {
			scanForNewJsonlFiles(
				dir, knownJsonlFiles, activeAgentIdRef, nextAgentIdRef,
...
export function autoDetectActiveAgents(
	projectDir: string,
	knownJsonlFiles: Set<string>,
	nextAgentIdRef: { current: number },
	agents: Map<number, AgentState>,
	...
	for (const dir of dirsToScan) {
		...
		// Derive folder name from project dir hash for subdirectory agents
		const dirBasename = path.basename(dir);
		const mainDirBasename = path.basename(projectDir);
		const isSubdir = dirBasename !== mainDirBasename;
		const folderName = isSubdir ? resolveFolderLabel(dirBasename, mainDirBasename) : undefined;
		...
		for (const file of files) {
			...
			// Try to find an unowned terminal to associate
			let matchedTerminal: vscode.Terminal | undefined;
			...
			agents.set(id, agent);
			persistAgents();
			console.log(`[Pixel Agents] Agent ${id}: auto-detected from ${path.basename(file)} in ${dirBasename}${matchedTerminal ? ` (terminal: "${matchedTerminal.name}")` : ' (detached)'}`);
...
/** Extract a readable subfolder label from a project dir hash relative to the main workspace hash.
 *  e.g. main="G--GitHub-nextspark", sub="G--GitHub-nextspark-projects-kuore" → "projects/kuore" */
function resolveFolderLabel(subDirHash: string, mainDirHash: string): string {
	const suffix = subDirHash.slice(mainDirHash.length + 1); // skip the leading dash
	return suffix.replace(/-/g, path.sep);
}
