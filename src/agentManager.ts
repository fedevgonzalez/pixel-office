...
/** Returns all project dirs matching the workspace or its subdirectories */
export function getProjectDirPaths(cwd?: string): string[] {
	const workspacePath = cwd || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
	if (!workspacePath) return [];
	const dirPrefix = workspacePath.replace(/[^a-zA-Z0-9-]/g, '-');
	...
	try {
		const dirs = fs.readdirSync(projectsRoot)
			.filter(d => {
				...
				return dLower === dirPrefixLower || dLower.startsWith(dirPrefixLower + '-');
			})
			.map(d => path.join(projectsRoot, d))
			.filter(d => { try { return fs.statSync(d).isDirectory(); } catch { return false; } });
		console.log(`[Pixel Agents] Project dirs for ${workspacePath}: ${dirs.map(d => path.basename(d)).join(', ')}`);
		return dirs;
	} catch { return []; }
}
...
export function persistAgents(
	...
	for (const agent of agents.values()) {
		persisted.push({
			id: agent.id,
			terminalName: agent.terminalRef?.name ?? `detached-${agent.id}`,
