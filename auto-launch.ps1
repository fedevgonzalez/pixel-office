# Auto-launch Pixel Office standalone server when Claude Code is running.
...
try {
    while ($true) {
        ...
        if ($claudeRunning -and -not $wasRunning) {
            ...
            Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Claude Code detected, starting Pixel Office server..."
        ...
        elseif (-not $claudeRunning -and $wasRunning) {
            ...
            if (-not $claudeRunning) {
                Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Claude Code stopped, shutting down Pixel Office server..."
